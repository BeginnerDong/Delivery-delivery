	...
	<div>
		...
		<div class="pdlr4">
			<div class="myaddress-lis boxShaow radius10" v-for="(item,index) in mainData">
				...
				<div class="seltBox">
					<div class="L" :data-id="item.id" @click="updateCard($event.currentTarget.dataset.id)">
						<img class="icon" :src="item.isdefault==1?'../../static/images/coupons-icon.png':'../../static/images/coupons-icon1.png'">
						默认银行卡
					</div>
					<div class="R fs12">
						<a class="child" :data-id="item.id" @click="Router.navigateTo({route:{path:'/pages/user_myBankCard_add/user_myBankCard_add?id='+$event.currentTarget.dataset.id}})">
							<img  src="../../static/images/address-icon.png" mode=""></image>编辑
						</a>
						<div class="child" :data-id="item.id" @click="deleteCard($event.currentTarget.dataset.id)">
							<img  src="../../static/images/address-icon1.png" mode=""></image>删除
						</div>
					</div>
				</div>
			</div>
			...
			<div class="submitbtn" style="margin-top: 80px;">
				<a class="btn"  @click="Router.navigateTo({route:{path:'/pages/user_myBankCard_add/user_myBankCard_add'}})">添加银行卡</a>
			</div>
		</div>
	</div>
</template>
